import { logs, recoveryLogs, sessions } from '../lib/store.js';
import { todayISO } from '../lib/models.js';

function statusFor(dateKey, todayKey) {
    ...
    <div className="calendar">
      {cells.map(c => (
        <div key={c.key} className={`day ${c.isFuture && c.status === 'open' ? 'future' : c.status} ${c.isToday ? 'today-marker' : ''}`}>
          <b>{c.dayNumber}</b>
          {c.isFuture ? 'Open' : c.status === 'trained' ? 'Trained' : c.status === 'recovery' ? 'Recovery' : c.status === 'partial' ? 'Partial' : c.status === 'missed' ? 'Missed' : 'Open'}
        </div>
      ))}
    </div>
  );
